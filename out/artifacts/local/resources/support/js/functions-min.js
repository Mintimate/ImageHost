! function(t, e, i, n) {
	...
	function A(t, e, i) {
		var n = i.pointers.length,
			...
			r = e & Pt && n - s === 0,
			o = e & (Rt | Mt) && n - s === 0;
		i.isFirst = !!r, i.isFinal = !!o, r && (t.session = {}), i.eventType = e, D(t, i), t.emit("hammer.input", i), t.recognize(
	...
	function D(t, e) {
		var i = t.session,
			n = e.pointers,
			s = n.length;
		i.firstInput || (i.firstInput = b(e)), s > 1 && !i.firstMultiple ? i.firstMultiple = b(e) : 1 === s && (i.firstMultiple = !
			1);
		var r = i.firstInput,
			o = i.firstMultiple,
			a = o ? o.center : r.center,
			l = e.center = P(n);
		e.timeStamp = gt(), e.deltaTime = e.timeStamp - r.timeStamp, e.angle = z(a, l), e.distance = M(a, l), _(i, e), e.offsetDirection =
			R(e.deltaX, e.deltaY);
		var c = O(e.deltaTime, e.deltaX, e.deltaY);
		e.overallVelocityX = c.x, e.overallVelocityY = c.y, e.overallVelocity = mt(c.x) > mt(c.y) ? c.x : c.y, e.scale = o ?
			X(o.pointers, n) : 1, e.rotation = o ? N(o.pointers, n) : 0, e.maxPointers = i.prevInput ? e.pointers.length > i.prevInput
			.maxPointers ? e.pointers.length : i.prevInput.maxPointers : e.pointers.length, S(i, e);
		var h = t.element;
	...
	function J(t) {
		this.options = Tt({}, this.defaults, t || {}), this.id = E(), this.manager = null, this.options.enable = u(this.options
			.enable, !0), this.state = fe, this.simultaneous = {}, this.requireFail = []
	}
	...
	function ct(t, e) {
		this.options = Tt({}, lt.defaults, e || {}), this.options.inputTarget = this.options.inputTarget || t, this.handlers = {},
			this.session = {}, this.recognizers = [], this.oldCssProps = {}, this.element = t, this.input = w(this), this.touchAction =
			new G(this, this.options.touchAction), ht(this, !0), o(this.options.recognizers, function(t) {
	...
	l(Y, I, {
		handler: function De(t) {
			var e = Vt[t.type];
			e & Pt && 0 === t.button && (this.pressed = !0), e & Ot && 1 !== t.which && (e = Rt), this.pressed && (e & Rt &&
	...
	t.MSPointerEvent && !t.PointerEvent && (Zt = "MSPointerDown", Jt = "MSPointerMove MSPointerUp MSPointerCancel"), l(k,
		I, {
			handler: function _e(t) {
				...
				s & Pt && (0 === t.button || o) ? 0 > a && (e.push(t), a = e.length - 1) : s & (Rt | Mt) && (i = !0), 0 > a || (e[
					a] = t, this.callback(this.manager, s, {
					pointers: e,
					changedPointers: [t],
					pointerType: r,
					srcEvent: t
	...
	G.prototype = {
		set: function(t) {
			t == ae && (t = this.compute()), oe && this.manager.element.style && pe[t] && (this.manager.element.style[re] = t),
	...
	J.prototype = {
		...
		emit: function(t) {
			...
			var i = this,
				n = this.state;
			ge > n && e(i.options.event + K(n)), e(i.options.event), t.additionalEvent && e(t.additionalEvent), n >= ge && e(i
				.options.event + K(n))
		},
		tryEmit: function(t) {
		...
		recognize: function(t) {
			var e = Tt({}, t);
			return h(this.options.enable, [this, e]) ? (this.state & (Te | ye | Ce) && (this.state = fe), this.state = this.process(
				e), void(this.state & (ve | me | ge | ye) && this.tryEmit(e))) : (this.reset(), void(this.state = Ce))
		},
		process: function(t) {},
		...
		directionTest: function(t) {
			var e = this.options,
				...
				r = t.deltaX,
				o = t.deltaY;
			return s & e.direction || (e.direction & qt ? (s = 0 === r ? zt : 0 > r ? Nt : Xt, i = r != this.pX, n = Math.abs(
					t.deltaX)) : (s = 0 === o ? zt : 0 > o ? Yt : kt, i = o != this.pY, n = Math.abs(t.deltaY))), t.direction = s,
		...
		emit: function(t) {
			this.state === Te && (t && t.eventType & Rt ? this.manager.emit(this.options.event + "up", t) : (this._input.timeStamp =
		...
		attrTest: function(t) {
			...
			return e & (qt | Ft) ? i = t.overallVelocity : e & qt ? i = t.overallVelocityX : e & Ft && (i = t.overallVelocityY),
				this._super.attrTest.call(this, t) && e & t.offsetDirection && t.distance > this.options.threshold && t.maxPointers ==
				this.options.pointers && mt(i) > this.options.velocity && t.eventType & Rt
		},
		emit: function(t) {
	...
	ct.prototype = {
		set: function(t) {
			return Tt(this.options, t), t.touchAction && this.touchAction.update(), t.inputTarget && (this.input.destroy(),
		...
		recognize: function(t) {
			...
			if (!e.stopped) {
				...
				for (var r = 0; r < n.length;) i = n[r], e.stopped === xe || s && i != s && !i.canRecognizeWith(s) ? i.reset() :
					i.recognize(t), !s && i.state & (ve | me | ge) && (s = e.curRecognizer = i), r++
			}
		},
		get: function(t) {
	...
	function t(t) {
		var n = $(".side-nav").find(".is-active"),
			...
			o = 0;
		"swipeup" === t.type || 40 === t.keyCode || t > 0 ? s !== r ? (o = s + 1, e(o), i(s, o, r)) : (e(o), i(s, o, r)) :
			("swipedown" === t.type || 38 === t.keyCode || 0 > t) && (0 !== s ? (o = s - 1, e(o), i(s, o, r)) : (o = r, e(o),
				i(s, o, r)))
	}

	function e(t) {
		$(".side-nav, .outer-nav").children().removeClass("is-active"), $(".side-nav").children().eq(t).addClass(
			"is-active"), $(".outer-nav").children().eq(t).addClass("is-active")
	}

	function i(t, e, i) {
		$(".main-content").children().removeClass("section--is-active"), $(".main-content").children().eq(e).addClass(
				"section--is-active"), $(".main-content .section").children().removeClass("section--next section--prev"), t ===
			i && 0 === e || 0 === t && e === i ? $(".main-content .section").children().removeClass(
				"section--next section--prev") : e > t ? $(".main-content").children().eq(t).children().addClass("section--next") :
			$(".main-content").children().eq(t).children().addClass("section--prev"), 0 !== e && e !== i ? $(".header--cta").addClass(
				"is-active") : $(".header--cta").removeClass("is-active")
	}
	...
	function s() {
		$(".slider--prev, .slider--next").click(function() {
			...
			$(".slider").animate({
				...
				t.hasClass("slider--next") ? a - 1 > i && a - 1 > s && a - 1 > o ? (l.removeClass("slider--item-left").next()
						.addClass("slider--item-left"), c.removeClass("slider--item-center").next().addClass("slider--item-center"),
						h.removeClass("slider--item-right").next().addClass("slider--item-right")) : i === a - 1 ? (u.removeClass(
						"slider--item-left").first().addClass("slider--item-left"), c.removeClass("slider--item-center").next().addClass(
						"slider--item-center"), h.removeClass("slider--item-right").next().addClass("slider--item-right")) : s ===
					a - 1 ? (l.removeClass("slider--item-left").next().addClass("slider--item-left"), u.removeClass(
							"slider--item-center").first().addClass("slider--item-center"), h.removeClass("slider--item-right").next()
						.addClass("slider--item-right")) : (l.removeClass("slider--item-left").next().addClass("slider--item-left"),
						c.removeClass("slider--item-center").next().addClass("slider--item-center"), u.removeClass(
							"slider--item-right").first().addClass("slider--item-right")) : 0 !== i && 0 !== s && 0 !== o ? (l.removeClass(
						"slider--item-left").prev().addClass("slider--item-left"), c.removeClass("slider--item-center").prev().addClass(
						"slider--item-center"), h.removeClass("slider--item-right").prev().addClass("slider--item-right")) : 0 ===
					i ? (u.removeClass("slider--item-left").last().addClass("slider--item-left"), c.removeClass(
						"slider--item-center").prev().addClass("slider--item-center"), h.removeClass("slider--item-right").prev().addClass(
						"slider--item-right")) : 0 === s ? (l.removeClass("slider--item-left").prev().addClass("slider--item-left"),
						u.removeClass("slider--item-center").last().addClass("slider--item-center"), h.removeClass(
							"slider--item-right").prev().addClass("slider--item-right")) : (l.removeClass("slider--item-left").prev().addClass(
						"slider--item-left"), c.removeClass("slider--item-center").prev().addClass("slider--item-center"), u.removeClass(
						"slider--item-right").last().addClass("slider--item-right"))
			}, 400), $(".slider").animate({
				opacity: 1
			}, 400)
		})
	...
	$(this).on("mousewheel DOMMouseScroll", function(e) {
		...
		var t = $(".side-nav").find(".is-active"),
			...
			r = s;
		e(s), i(n, r, s)
	});$(".ToWork2").click(function() {
		var t = $(".side-nav").find(".is-active"),
			n = $(".side-nav").children().index(t),
			s = $(".side-nav").children().length - 4,
			r = s;
		e(s), i(n, r, s)
	});$(".ToWork3").click(function() {
		var t = $(".side-nav").find(".is-active"),
			n = $(".side-nav").children().index(t),
			s = $(".side-nav").children().length - 3,
			r = s;
		e(s), i(n, r, s)
	});$(".ToWork4").click(function() {
		var t = $(".side-nav").find(".is-active"),
			n = $(".side-nav").children().index(t),
